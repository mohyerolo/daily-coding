...
public class 벽부수고이동하기_3 {
    private static class Space {
        ...
        int broken, moveCnt;
        boolean night;
        Space(final int row, final int col, final int broken, final int moveCnt, final boolean night) {
            this.row = row;
            this.col = col;
            this.broken = broken;
    ...
    private static int bfs() {
        queue.offer(new Space(0, 0, 0, 1, false));
        visit[0][0] = 0;
    ...
    private static void move(final int row, final int col, final Space space) {
        if (map[row][col] == '0') {
            ...
            queue.offer(new Space(row, col, space.broken, space.moveCnt + 1, !space.night));
            return;
        }

        if (space.broken < K) {
            breakWall(row, col, space);
        }
    }

    private static void breakWall(final int row, final int col, final Space space) {
        if (space.night) {
            queue.offer(new Space(space.row, space.col, space.broken, space.moveCnt + 1, !space.night));
            return;
        }
        visit[row][col] = space.broken + 1;
        queue.offer(new Space(row, col, space.broken + 1, space.moveCnt + 1, !space.night));
    }
}
